...
Example:
    >>> import openpkpd
    >>> from openpkpd.import_ import import_nonmem, import_monolix
    >>> openpkpd.init_julia()
    >>>
    >>> # Import NONMEM model
...
class ImportedModel:
    ...
    Attributes:
        ...
        model_kind: OpenPKPD model type name
...
def get_model_mapping(advan: int, trans: int) -> Optional[Tuple[str, List[str]]]:
    """
    Get the OpenPKPD model kind and expected parameters for ADVAN/TRANS.
...
def get_monolix_model_mapping(model_name: str) -> Optional[str]:
    ...
    Returns:
        OpenPKPD model kind or None if not supported
    """
...
def import_nonmem(
    ...
    if doses:
        for d in doses:
            dose = jl.OpenPKPDCore.DoseEvent(
...
def parse_nonmem_control(path: Union[str, Path]) -> NONMEMControlFile:
    ...
    result = jl.OpenPKPDCore.parse_nonmem_control(ctl_text)
...
def parse_nonmem_control_text(text: str) -> NONMEMControlFile:
    ...
    """
    jl = _require_julia()
    result = jl.OpenPKPDCore.parse_nonmem_control(text)
...
def parse_monolix_project(path: Union[str, Path]) -> MonolixProject:
    ...
    result = jl.OpenPKPDCore.parse_monolix_project(mlx_text)
...
def parse_monolix_project_text(text: str) -> MonolixProject:
    ...
    """
    jl = _require_julia()
    result = jl.OpenPKPDCore.parse_monolix_project(text)
