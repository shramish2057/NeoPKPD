"""
OpenPKPD Trial Core Integration
...
def _get_julia():
    """Get the Julia connection from openpkpd."""
    try:
        ...
        raise RuntimeError(
            ...
            "and the openpkpd package is properly configured."
        )
...
def simulate_subject_exposure(
    ...
    if jl_model_name == 'OneCompartmentIVBolus':
        pk_model = jl.OpenPKPDCore.OneCompartmentIVBolus()
        pk_params_jl = jl.OpenPKPDCore.OneCompartmentIVBolusParams(
            CL=pk_params.get('CL', 10.0),
            V=pk_params.get('V', 50.0)
        )
    elif jl_model_name == 'OneCompartmentOral':
        pk_model = jl.OpenPKPDCore.OneCompartmentOral()
        pk_params_jl = jl.OpenPKPDCore.OneCompartmentOralParams(
            CL=pk_params.get('CL', 10.0),
            V=pk_params.get('V', 50.0),
        ...
        pk_model = jl.OpenPKPDCore.TwoCompartmentIVBolus()
        pk_params_jl = jl.OpenPKPDCore.TwoCompartmentIVBolusParams(
            CL=pk_params.get('CL', 10.0),
        ...
        pk_model = jl.OpenPKPDCore.TwoCompartmentOral()
        pk_params_jl = jl.OpenPKPDCore.TwoCompartmentOralParams(
            CL=pk_params.get('CL', 10.0),
            V1=pk_params.get('V1', 50.0),
            Q=pk_params.get('Q', 5.0),
            V2=pk_params.get('V2', 100.0),
    ...
    model_spec = jl.OpenPKPDCore.PKOnlySpec(pk_model, pk_params_jl)

    # Build dosing events
    doses = jl.seval("OpenPKPDCore.DoseEvent[]")
    for dose in dose_events:
        dose_event = jl.OpenPKPDCore.DoseEvent(
            float(dose['time']),
            float(dose['amount']),
    ...
    if grid is None:
        ...
        grid_jl = jl.OpenPKPDCore.SimGrid(0.0, t_max, obs_times)
    else:
    ...
    # Run simulation
    result = jl.OpenPKPDCore.simulate(model_spec, doses, grid=grid_jl, solver=solver_jl)
