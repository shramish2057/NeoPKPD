...
def bioequivalence_90ci_design(
    ...
    if design == BEStudyDesign.CROSSOVER_2X2:
        jl_design = jl.OpenPKPDCore.Crossover2x2()
    elif design == BEStudyDesign.CROSSOVER_2X4:
        jl_design = jl.OpenPKPDCore.Crossover2x4()
    else:
        jl_design = jl.OpenPKPDCore.ParallelGroup()

    result = jl.OpenPKPDCore.bioequivalence_90ci_design(
        t, r, jl_design,
...
def compute_be_degrees_of_freedom(
    ...
    return int(jl.OpenPKPDCore.compute_be_degrees_of_freedom(jl_design, n))
...
def compute_swr(
    ...
    return float(jl.OpenPKPDCore.compute_swr(r1, r2))
...
def compute_within_subject_variance_reference(
    ...
    result = jl.OpenPKPDCore.compute_within_subject_variance_reference(ref_obs_jl)
...
def rsabe_analysis(
    ...
    data = jl.OpenPKPDCore.ReplicateData(
    ...
    if config.guidance == RegulatoryGuidance.FDA:
        guidance = jl.OpenPKPDCore.FDAGuidance()
    elif config.guidance == RegulatoryGuidance.EMA:
        guidance = jl.OpenPKPDCore.EMAGuidance()
    else:
        guidance = jl.OpenPKPDCore.HealthCanadaGuidance()

    # Create design
    if config.design == ReplicateDesign.PARTIAL_REPLICATE_3X3:
        design = jl.OpenPKPDCore.PartialReplicate3x3()
    elif config.design == ReplicateDesign.FULL_REPLICATE_2X4:
        design = jl.OpenPKPDCore.FullReplicate2x4()
    else:
        design = jl.OpenPKPDCore.FullReplicate2x3()

    # Create config
    rsabe_config = jl.OpenPKPDCore.RSABEConfig(
        guidance=guidance,
        design=design,
    ...
    result = jl.OpenPKPDCore.rsabe_analysis(data, rsabe_config)
...
def abel_analysis(
    ...
    result = jl.OpenPKPDCore.abel_analysis(data, rsabe_config)
...
def abel_scaled_limits(swr: float) -> Dict[str, Any]:
    ...
    jl = _require_julia()
    guidance = jl.OpenPKPDCore.EMAGuidance()
    result = jl.OpenPKPDCore.abel_scaled_limits(swr, guidance)
