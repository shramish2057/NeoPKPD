...
def _build_population_spec(jl, spec: Dict[str, Any]):
    ...
    if kind_str == "OneCompIVBolus":
        ...
        model_params = jl.OpenPKPDCore.OneCompartmentIVBolusParams(
            CL=float(params["CL"]),
            V=float(params["V"])
        )
    elif kind_str in ["OneCompOral", "OneCompOralFirstOrder"]:
        pk_model = jl.OpenPKPDCore.OneCompartmentOral()
        model_params = jl.OpenPKPDCore.OneCompartmentOralParams(
            CL=float(params["CL"]),
            V=float(params["V"]),
            Ka=float(params["Ka"]),
        ...
        model_params = jl.OpenPKPDCore.TwoCompartmentIVBolusParams(
            CL=float(params["CL"]),
            V1=float(params["V1"]),
            Q=float(params["Q"]),
            V2=float(params["V2"])
        ...
        model_params = jl.OpenPKPDCore.TwoCompartmentOralParams(
            CL=float(params["CL"]),
            V1=float(params["V1"]),
            Q=float(params["Q"]),
            V2=float(params["V2"]),
    ...
    model_spec = jl.OpenPKPDCore.PKOnlySpec(pk_model, model_params)
...
def _build_error_spec(jl, spec: Dict[str, Any]):
    ...
    if error_type == "additive":
        ...
        params = jl.OpenPKPDCore.CombinedErrorParams(
            float(sigma),
...
def compute_vpc(
    ...
    if isinstance(config.binning, QuantileBinning):
        binning = jl.OpenPKPDCore.QuantileBinning(config.binning.n_bins)
    elif isinstance(config.binning, EqualWidthBinning):
        binning = jl.OpenPKPDCore.EqualWidthBinning(config.binning.n_bins)
    elif isinstance(config.binning, KMeansBinning):
        binning = jl.OpenPKPDCore.KMeansBinning(config.binning.n_bins)
    else:
        binning = jl.OpenPKPDCore.QuantileBinning(10)
