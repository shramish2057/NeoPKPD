"""
OpenPKPD Sensitivity Analysis Visualization
...
def plot_tornado(
    ...
    if backend == "matplotlib":
        ...
        bars_low = ax.barh(y_pos, low_delta, height=0.6,
                           color=OPENPKPD_COLORS["primary"], alpha=0.8,
                           label='-20% Parameter')

        # High side
        bars_high = ax.barh(y_pos, high_delta, height=0.6,
                            color=OPENPKPD_COLORS["secondary"], alpha=0.8,
                            label='+20% Parameter')
        ...
        fig.add_trace(go.Bar(
            ...
            orientation='h',
            name='-20% Parameter',
            marker_color=OPENPKPD_COLORS["primary"]
        ))
        ...
        fig.add_trace(go.Bar(
            ...
            orientation='h',
            name='+20% Parameter',
            marker_color=OPENPKPD_COLORS["secondary"]
        ))
...
def plot_waterfall(
    ...
    colors = [OPENPKPD_COLORS["success"] if v >= 0 else OPENPKPD_COLORS["error"]
              for v in values]
    ...
    if backend == "matplotlib":
        ...
        ax.bar(x_pos[0], baseline_value, color=OPENPKPD_COLORS["info"], width=0.6)
        ax.bar(x_pos[-1], cumulative[-1], color=OPENPKPD_COLORS["dark"], width=0.6)
        ...
        fig = go.Figure(go.Waterfall(
            ...
            measure=measures,
            y=vals,
            decreasing=dict(marker_color=OPENPKPD_COLORS["error"]),
            increasing=dict(marker_color=OPENPKPD_COLORS["success"]),
            totals=dict(marker_color=OPENPKPD_COLORS["dark"]),
...
def plot_one_at_a_time(
    ...
    Example:
        >>> from openpkpd import run_sensitivity
