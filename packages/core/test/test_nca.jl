...
@testset "BE Confidence Interval - T-Distribution" begin
    ...
    @testset "T-CDF and T-PDF accuracy" begin
        ...
        @test OpenPKPDCore._t_cdf(0.0, 10) ≈ 0.5 atol=1e-10  # Symmetric at 0
        @test OpenPKPDCore._t_cdf(Inf, 10) ≈ 1.0 atol=1e-10  # Limit at infinity
        @test OpenPKPDCore._t_cdf(-Inf, 10) ≈ 0.0 atol=1e-10  # Limit at -infinity

        # Verify PDF properties
        @test OpenPKPDCore._t_pdf(0.0, 10) > 0.0  # Positive at peak
        @test OpenPKPDCore._t_pdf(0.0, 10) > OpenPKPDCore._t_pdf(1.0, 10)  # Peak at 0
        @test OpenPKPDCore._t_pdf(-1.0, 10) ≈ OpenPKPDCore._t_pdf(1.0, 10) atol=1e-10  # Symmetric

        # Known CDF values for df=10
        # P(T <= 1.812) ≈ 0.95 for df=10
        @test OpenPKPDCore._t_cdf(1.812, 10) ≈ 0.95 atol=0.01
    end
end
