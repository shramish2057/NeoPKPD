...
@testset "IIV Covariance" begin
    ...
    @testset "Correlated Eta Sampling" begin
        @testset "Sampling Produces Correct Covariance Structure" begin
            ...
            for i in 1:n_samples
                eta_vec, _ = OpenPKPDCore.sample_eta_vector(om, rng)
                etas_CL[i] = eta_vec[1]
                etas_V[i] = eta_vec[2]
            end
        ...
        @testset "Independent Sampling with Diagonal" begin
            ...
            rng = StableRNG(12345)
            for i in 1:n_samples
                eta_vec, param_names = OpenPKPDCore.sample_eta_vector(om, rng)
        ...
        @testset "Reproducibility with Seed" begin
            ...
            rng1 = StableRNG(42)
            eta1, _ = OpenPKPDCore.sample_eta_vector(om, rng1)

            rng2 = StableRNG(42)
            eta2, _ = OpenPKPDCore.sample_eta_vector(om, rng2)
    ...
    @testset "Serialization Round-Trip" begin
        @testset "Diagonal IIVSpec" begin
            ...
            # Serialize
            serialized = OpenPKPDCore._serialize_iiv(iiv_orig)
            ...
            # Deserialize
            iiv_parsed = OpenPKPDCore._parse_iiv(serialized)
