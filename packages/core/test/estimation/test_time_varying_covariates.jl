...
@testset "Time-Varying Covariates" begin
    ...
    @testset "Transform Application" begin
        @testset "NoTransform" begin
            @test OpenPKPDCore.apply_transform(70.0, NoTransform()) == 70.0
        end

        @testset "LogTransform" begin
            transform = LogTransform(70.0)
            @test isapprox(OpenPKPDCore.apply_transform(70.0, transform), 0.0, atol=1e-10)
            @test isapprox(OpenPKPDCore.apply_transform(140.0, transform), log(2.0), atol=1e-10)
        end

        @testset "PowerTransform" begin
            transform = PowerTransform(70.0, 0.75)
            @test isapprox(OpenPKPDCore.apply_transform(70.0, transform), 1.0, atol=1e-10)
            @test isapprox(OpenPKPDCore.apply_transform(140.0, transform), 2.0^0.75, atol=1e-10)
        end

        @testset "NormalizedTransform" begin
            transform = NormalizedTransform(100.0, 50.0)
            @test isapprox(OpenPKPDCore.apply_transform(100.0, transform), 0.0, atol=1e-10)
            @test isapprox(OpenPKPDCore.apply_transform(150.0, transform), 1.0, atol=1e-10)
            @test isapprox(OpenPKPDCore.apply_transform(50.0, transform), -1.0, atol=1e-10)
        end
    end
